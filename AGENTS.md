...
决策状态使用 `已确认 / 当前 / 待确认 / 受影响 / 已失效 / 阻塞`。用户要求回到某个决策时，只能重新确认当前对话决策，不删除历史，也不表示 Git 回退；确认原决定时恢复为 `已确认`，用户刚提出新决定时为 `当前`，用户确认新决定后为 `已确认`，默认假设为 `待确认`，直接依赖项为 `受影响`，间接依赖项为 `待确认`，未完成重新确认前不得进入正式输出。医疗用途、患者数据、自动诊断、正式报告、人工复核、病理诊断、治疗建议、ICU 和高风险报警等决策改变时，必须重新执行安全红线检查。
