...
<p align="center">
  <em>说不清的医疗需求,它替你一点一点问清楚,然后产出向交付级靠拢的方案。</em>
</p>
...
| **首轮澄清** | 新项目 / 需求模糊 | 走完整追问流程,产出架构概要 + 决策表 + 自检表 |
| **迭代轮** | 已有澄清基础,只做增量修改 | 只输出"改了什么 + 验证结果",不重复问 |

**首轮澄清时选模式:**

- **教学模式**:产出演示性实现,侧重展示医疗暗规则如何影响设计。适合学习/原型。
- **落地模式(默认)**:产出向交付级靠拢的可运行代码,交付时明确"上线缺口清单"(合规注册/临床验证/安全测试)。
...
flowchart TD
    ...
    C --> D[点破隐含矛盾]
    D --> E[补全领域现实]
    E --> F{需求已收敛?<br/>能说清数据流/模块/接口}
    F -- 否 --> B
    F -- 是 --> G[输出方案]
```

- **追问规则**:小问题、具体到界面/操作、一次一个、医学术语用大白话、按优先级、拆目标、收敛就停。
- **提问框架**:通用(目标→用户→主流程→数据→边界→交付),叠加扫描/影像、UI/登录、病理学、AI辅助诊断、ICU实时监测五个品类。
...
它比通用模型多知道的医疗现实,沉淀在 `references/` 下 12 类、151 条规则 —— 每条都标注"为什么问(医学现实)+ 踩过什么坑(真实事故)",并在追问时用它点破需求里的隐含矛盾。
...
| 病理学与数字病理 | 制片链路/切片质量/对照/切缘/双重阅片/WSI |
| AI 辅助诊断与 SaMD | 用途定位/临床评价/人机协同/可解释/版本治理/漂移监控 |
| ICU 实时监测与报警 | 采样率/伪差/报警分级/报警疲劳/失效安全/审计 |

**这是通用 AI 抄不走的护城河** —— 抄一套追问流程容易,积累 12 类医疗暗规则难。它来自真实医疗器械开发(MC-01 骨髓分析仪、精子分析、血细胞分析等真实项目)踩过的坑。
